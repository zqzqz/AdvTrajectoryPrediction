...
def draw():
    ...
    normal_result_data = load_data("case_study/grip-apolloscape-121-4-left-n/normal.json")
    attack_result_data = load_data("case_study/grip-apolloscape-121-4-left-n/attack.json")
    defense_result_data = load_data("case_study/grip-apolloscape-121-4-left-n/attack_smooth2.json")
    ...
    defense_preds = [np.concatenate((defense_result_data["output_data"][str(i)]["objects"]["4"]["observe_trace"][-1,:].reshape(1,2), defense_result_data["output_data"][str(i)]["objects"]["4"]["predict_trace"]), axis=0) for i in range(6)]

    theta = -13/180*np.pi
    center = np.array([470, 110])
    ...
    plt.savefig("figures/case-study.pdf", bbox_inches='tight')
